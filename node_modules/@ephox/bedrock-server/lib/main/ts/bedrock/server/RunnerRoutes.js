...
const generate = async (mode, projectdir, basedir, configFile, bundler, testfiles, chunk, retries, singleTimeout, stopOnFailure, basePage, coverage, polyfills) => {
    const files = testfiles.map((filePath) => {
        return path.relative(projectdir, filePath);
    });
    const testGenerator = Compiler.compile(path.join(projectdir, configFile), path.join(projectdir, 'scratch'), basedir, mode === 'auto', files, coverage, polyfills);
    ...
    const findWorkspaceResources = (moduleFolder) => {
    ...
    const workspaceRoots = (pkjson.workspaces
...
//# sourceMappingURL=RunnerRoutes.js.map